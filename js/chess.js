function Chess(){
	...
	/*起始棋子*/
	function start()//棋子初始化
	{
		if("turnover"==nameBak)//翻转棋
		{
			moreMan([3+"_"+3,4+"_"+3,4+"_"+4,3+"_"+4]);
		}
		else 
			if("four"==nameBak)
			{
				man(minL,minL,nameBak+"_"+row/2+"_"+0);//四子棋
		}
		else 
			if("gogame"==nameBak)
			{
				moreMan([3+"_"+3,15+"_"+3,15+"_"+15,3+"_"+15]);//围棋
		}
	}
	/*画棋盘table标签*/
	this.board=function(name,width,height,output)
	{
		...
		for(var i=0;i<row;i++)
		{
			...
			for(var j=0;j<col;j++)
			  {
				  array.push("<td align=center>"+evt(i,j,minL,minL,minL/2*-1-4,minL/2*-1-4)+"</td>");
			  }
			array.push("</tr>");
		}
		...
		if(nameBak!="four"&&nameBak!="turnover")//将事件添加到表格中
		  {
			  for(var j=0;j<col;j++)
		     {
				 array.push(evt(row,j,minL,minL+2,minL/2*-1-2,minL/2*-1-4));
			 }
		  }
		...
		if(nameBak!="four"&&nameBak!="turnover")
		{
			for(var j=0;j<row;j++)
			{
				array.push(evt(j,col,minL+2,minL,minL/2*-1-4,minL/2*-1-2));
			}
		}
	...
	function rule()
	{
		...
		if(id.indexOf("man_")==0)
		{
			alert("不能在有子的地方落子");return false;
		}
		else
		{
			...
			{
				if(isMan(row,col+1)==null&&isMan(row,col-1)==null&&isMan(row+1,col)==null&&isMan(row-1,col)==null)
				{
					alert("四子棋不能在四周空白的地方落子！");return false;
				}
				return gobang(row,col,3);
			}
			else if("turnover"==nameBak)
			{
				if(isMan(row,col+1)==null&&isMan(row,col-1)==null&&isMan(row+1,col)==null&&isMan(row-1,col)==null&&isMan(row-1,col-1)==null&&isMan(row+1,col+1)==null)
				{
					alert("翻转棋不能在四周空白的地方落子！");return false;
				}
				turnover();
			}
	...
	/*判断是否有棋子*/
	function isMan(row,col)
	{
		var obj=_$(nameBak+"_"+row+"_"+col,1);if(obj==null||obj.indexOf("man_")==-1)return null;else if(obj.indexOf("000")!=-1)return 0;else if(obj.indexOf("CCC")!=-1)return 1;
	}
	/*五子棋或者四子棋的输赢结果判断，五子棋是五子相连算赢，四子棋是四子相连算赢*/
	function gobang(row,col,num)
	{
		num=num==null?4:num;
		var rs=[[],[],[],[]],b=[],w=[];/*采用四维数组来存储棋子位置*/
	...
	function setClick(row,col,width,height)
	{
	    for(var i=0;i<=row;i++)
		{
			for(var j=0;j<=col;j++)
			{
				...
				if(els!=null)els.onclick=function()
				{
					if(rule())man(width,height);
				};
			}
	    }
	}
}
function _$(CHOFO,CHOF0)
{
	var CH0F0=document.getElementById(CHOFO);if(CH0F0!=null)
	{
		if(CHOF0==0)return CH0F0.style;else if(CHOF0==1)return CH0F0.innerHTML;
	}return CH0F0;
}
